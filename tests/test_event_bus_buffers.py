...
class TestEventBusBenchmark(unittest.TestCase):
    ...
    def test_generate_visualizations(self):
        """Execute benchmarks and generate matplotlib visualizations if available."""
        ...
        # 1. Processing Throughput: Sync vs Async
        def get_sync_throughput():
            bus = EventBus(buffer_size=num_events*2, batch_size=1000, worker_count=4)
            handler = CountingSyncHandler()
            bus.subscribe(handler)
            async def run_sync():
                await bus.start()
        ...
        def get_async_throughput():
            bus = EventBus(buffer_size=num_events*2, batch_size=256, worker_count=4)
            handler = NoOpAsyncHandler()
            bus.subscribe(handler)
            async def run_async():
                await bus.start()
        ...
        sync_rate = get_sync_throughput()
        async_rate = get_async_throughput()

        # 2. Batch Pop Speedup
        from sally.core.event_bus import RingBuffer
        def get_pop_rates():
            buf1 = RingBuffer(capacity=num_events * 2)
            buf2 = RingBuffer(capacity=num_events * 2)
            for _ in range(num_events):
                buf1.push(Event(event_type="test"))
                buf2.push(Event(event_type="test"))

            start1 = time.perf_counter()
            while not buf1.is_empty:
                buf1.pop()
            indiv_rate = num_events / (time.perf_counter() - start1)

            start2 = time.perf_counter()
            while not buf2.is_empty:
                buf2.pop_batch(256)
            batch_rate = num_events / (time.perf_counter() - start2)

            return indiv_rate, batch_rate

        indiv_rate, batch_rate = get_pop_rates()

        # 3. Publish vs Processing (Sync Handler)
        # Using data gathered during sync test methodology
        publish_events = 1_000_000
        events_list = [Event(event_type="bench", timestamp=time.time()) for _ in range(publish_events)]
        dummy_bus = EventBus(buffer_size=publish_events*2, batch_size=1000, worker_count=1)
        dummy_handler = NoOpSyncHandler()
        dummy_bus.subscribe(dummy_handler)

        start_pub = time.perf_counter()
        for e in events_list:
            dummy_bus.publish_sync(e)
        pure_publish_rate = publish_events / (time.perf_counter() - start_pub)
        ...
        ax3.set_ylabel('Events per Second', fontweight='bold')
        ax3.set_title('Event Bus Pipeline Bottleneck Analysis', fontsize=14, fontweight='bold')
        ax3.grid(True, axis='y', linestyle='--', alpha=0.7)
        ax3.set_yscale('log') # Log scale is often better for showing pure publish vs full processing
